...
public sealed class SpectralTimerCD : IConditionDefinition {
    ...
    public readonly record struct SpectralTimeParams(int Seconds, string Op, bool Invert) {
    ...
    public void DrawParams(Condition condition) {
        ...
        using var combo = ImRaii.Combo("##spectral_op", label);
        if (combo) {
    ...
    private static SpectralTimeParams GetParams(IReadOnlyDictionary<string, object> p) {
        var sec = GetDouble(p, "sec", 0);
        var op = GetOp(p, "op", ">=");
        var inv = GetBool(p, "inv", false);
        return new SpectralTimeParams((int)Math.Floor(sec), op, inv);
    }
}
